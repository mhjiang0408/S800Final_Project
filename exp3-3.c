...
void SysTick_Handler(void)
{
	...
	{
		systick_100ms_status	= systick_10ms_status = 0; //阻塞任务1和2
  ...
  UARTIntClear(UART0_BASE, uart0_int_status);							//Clear the asserted interrupts
	...
	if (uart0_int_status & (UART_INT_RX | UART_INT_RT)) 	//接收或接收超时
	{
		while(UARTCharsAvail(UART0_BASE))    							// Loop while there are characters in the receive FIFO.
		{
			...
			uart_receive_char[k++] = toupper(UARTCharGetNonBlocking(UART0_BASE));	//全部转换为大写字母
		}
		
		if(uart_receive_char[k-1] == '$')	//接收到结束标记$
		{
			uart_receive_char[k-1] = '\0';	//去除结束标记$
			receive_status = 1;	//\当PC端所发送字符串的所有字符都接收到了,将接收标志置1
			k = 0;
		}
	}
