...
volatile uint16_t systick_1ms_couter=0, systick_10ms_couter=0, systick_100ms_couter=0; //10ms??100ms???¡À?¡Â
volatile uint8_t	systick_1ms_status=0,systick_10ms_status=0, systick_100ms_status=0; //10ms??100ms???¡À¡Á???
...
uint8_t I2C0_ReadByte(uint8_t DevAddr, uint8_t RegAddr)
{
	...
	I2CMasterSlaveAddrSet(I2C0_BASE, DevAddr, false); //?¨¨???¨²???¡¤????
	I2CMasterDataPut(I2C0_BASE, RegAddr); //?¨¨???????¡¤
	I2CMasterControl(I2C0_BASE,I2C_MASTER_CMD_SINGLE_SEND); //????¡Á???¡¤???
	...
	I2CMasterControl(I2C0_BASE,I2C_MASTER_CMD_SINGLE_RECEIVE); //????¡Á???????
	while(I2CMasterBusBusy(I2C0_BASE));
...
{
	if (systick_100ms_couter == 0) //????1ms??SysTick?¨²?¨²100ms???¡§?¡À?¡Â
	{
	...
	if (systick_10ms_couter	== 0) //????1ms??SysTick?¨²?¨²10ms???¡§?¡À?¡Â
	{
		...
		if (systick_1ms_couter	== 0) //????1ms??SysTick?¨²?¨²1ms???¡§?¡À?¡Â
	{
	...
	{
		systick_100ms_status	= systick_10ms_status = 0; //¡Á¨¨??????1??2???¡Â??
...
//	char uart_receive_char;
  ...
  UARTIntClear(UART0_BASE, uart0_int_status);							//Clear the asserted interrupts
	...
	if (uart0_int_status & (UART_INT_RX | UART_INT_RT)) 	//?????¨°???????¡À
	{
...
void S800_QEI_Init(void)
{
  ...
  GPIOPinConfigure(GPIO_PL2_PHB0);
	//software patch to force the PL3 to low voltage, ??????C?¡§??PL3??¡À???????
...
void BootAnimation(void)
{
	...
	while(count2<1000)
	{
	  while(count1<1000)
	  {
		  if (systick_1ms_status) //1ms?¡§?¡À??
		  {
				systick_1ms_status	= 0; //????1ms?¡§?¡À¡Á???
				flag = flag%8+1;
